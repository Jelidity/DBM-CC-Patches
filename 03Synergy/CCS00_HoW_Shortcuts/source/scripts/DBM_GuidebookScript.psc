...
Quest Property DBM_HOSDiscoveryQST Auto
...
ObjectReference[] Property aTeleportLocations4 Auto
{Teleport locations from the fourth menu}

Bool FirstUse = TRUE
Bool GuildHouseTravel
Bool PlanetariumTravel
Bool SafeHouseTravel
Bool HallSecretsTravel
...
Event OnRead()	
	...
	;;Hall of Secrets unlocked
	if !HallSecretsTravel && DBM_HOSDiscoveryQST.IsCompleted()
		HallSecretsTravel = True
		DBM_GuideNewLocationMSG.Show()
	endif
	...
	elseif iNavOption == 1 ;Teleport
		...
		elseif iTeleportOption == 4 ;Next Menu
			...
			elseif iTeleportOption == 4 ;Next menu
				...
				if iTeleportOption < 4 ;DBHall, Guildhouse, Planetarium, Hall of Secrets
				...
				elseif iTeleportOption == 4 ;Next menu
				iTeleportOption = DBM_GuideMSG4.Show()
					if iTeleportOption < 1 ;Hall of Wonders
						TeleportDestination = aTeleportLocations4[iTeleportOption]
						if PlayerRef.GetParentCell() != TeleportDestination.GetParentCell()
							debug.TraceUser("LegacyoftheDragonborn", Self+" teleporting player to: "+TeleportDestination)
							PlayerRef.MoveTo(TeleportDestination)
						else
					...
					elseif iTeleportOption == 1 ;Cancel
						;
					endif
				endif
			endif
